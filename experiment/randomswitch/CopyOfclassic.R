...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(),
    mainPanel(
      randomizeInput("randomslider"),
      randomizeInput("randomslider2", label = "random another thing"),
      actionButton('wathever', "watherer")
    )
  )
)

server <- function(input, output, session) {
  callModule(randomize, "randomslider")
  callModule(randomize, "randomslider2")
