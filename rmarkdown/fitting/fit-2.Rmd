...
A fitting formula is very flexible; it can take any number of user-defined variables and they can be combined in almost any imaginable way (R functions can be used inside the formula).

`eflows` puts a prime on the usability of its API, so when working with `e_frame` objects the most important factors can be directly used in the formula using predefined variable names:
...
Following R conventions, the fitting formula is prefixed by `~`. In its simplest shape it takes a form like `~ 1* .price`. This means that, at any given moment, the flexible demand shall be allocated where the price of the energy is the lowest.

Note that to maximize the consumption of renewable energy the formula should be `~ -1* .production_fixed`; *negative*, because we want to allocate the demand when `.production_fixed` is the highest, not the lowest. 
...
When the function starts to run, the fitting curve is determined by the fitting formula. At this point, the only demand that is allocated is the fixed one, so `~ 1* .demand_fixed` and `~ 1* .demand` will produce identical curves. 

Every time a new bit of demand is allocated, the fitting curve is updated. This has consequences if the formula includes `.demand`, because the previously allocated flexible demand will will influence where is located the new lowest point of the fitting curve. 
...
This is why `.demand` acts a feedback mechanism. When the formula depends exclusively on it, it generates the "peak shaving profile"; when combined with other variables, it generates a "penalty factor" over those moments in time where a high amount of demand was already allocated. 

Note that when used `.demand` in the formula, in addition to the *initial* fitting curve there will be a *final* fitting curve. 
...
A detail not mentioned above is that `foreshift()` and `backshift()` allocate demand in the lowest point *among finite values* of the fitting curve. Because `NA` values are not finite, this prevents them to be assigned more demand. 
