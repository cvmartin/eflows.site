...
- **Renewable within a limit** (`ifelse(.demand < 60, (-.production_fixed), .demand)`): The grid cap can be included directly in the formula. In this case, whenever the total demand exceeds `60`, a point is disallowed to keep receiving demand.
...
- **Uniform layer** (`.demand - .demand_fixed`): It is also possible to assign energy ignoring all the factors except the difference between `.demand_fixed` and `.demand`, namely, the flexible demand. Because there is no reason to be assigned otherwise, the foreshifte profile will tend to form a smoothed layer over the fixed demand. 
