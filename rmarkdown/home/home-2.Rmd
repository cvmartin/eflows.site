...
This website, as `eflows` itself, is in active development and hence largely incomplete. Shorter term plans include the integration of functions `backshift()` and `simulate()`, and the consideration of heat pumps and batteries as user cases. Please consider subscribing to the mail list to keep updated about `eflows` releases. 

Righ now, the site aims to expose how `eflows` can be used to coordinate the charging of Electric Vehicles. The full explanation spans several articles, better read in this order: 

1. [Foreshift](.#foreshift): A first grasp of `eflows` possibilites and its take on flexible demand.
...
- [Design principles](.#principles): An additional article that delves into the architecture and the dessign behind `eflows`.
...
So, the best way to improve `eflows` is testing it; try to use it with your data, see what works and what not, and please provide feedback. You can always write me an email (LINK), although I'd rather have bug reports and request of new functionalities in the corresponding Github pages ([here](https://github.com/cvmartin/eflows/issues) and [here](https://github.com/cvmartin/eflows.viz/issues)). 

And of course, feel free to fork the repository, or propose changes through a merge[??] request.
